...
addMark(mark, subject) {
  ...
  getAverageBySubject(subject, mark) {
    if (this.marks[subject] === undefined) {
      return 0;
    }
    let averageMarkSubject= this.marks[subject].reduce((acc, mark, index, arr) => {
      acc += mark;
   ...
   getAverage() {
    ...
    let averageMarkSubject= allSubject.reduce((acc, mark, index, arr) => {
      acc += mark;
      if (index === arr.length - 1) {
          return acc / arr.length;
      }
